      ...
      {transaction.type == "listing" ? (
        <div className="flex gap-2 sm:flex-row flex-col">
          <div className="border-2 border-outline rounded-md flex flex-col items-center relative h-[120px] w-[120px] sm:h-[200px] sm:w-[200px] overflow-hidden p-2 bg-gradient-to-tr from-outline via-white to-white">
            <h1
              className={`absolute top-1 left-1 sm:text-xl text-sm ${
          ...
          {transaction.seller.id == user.id ? (
            <div className="text-black p-2 flex flex-col">
              <h1 className="text-xl sm:text-3xl">Sale</h1>
              <h2 className="text-sm sm:text-lg">
                {transaction.buyer.username} purchased your listing
              </h2>
              <h2 className="text-xl sm:text-3xl mt-auto text-green-400">
                ${transaction.info.price}
              </h2>
            </div>
          ) : (
            <div className="text-black p-2 flex flex-col">
              <h1 className="text-xl sm:text-3xl">Purchase</h1>
              <h2 className="text-sm sm:text-lg">
                You purchased {transaction.seller.username}'s listing
              </h2>
              <h2 className="text-xl sm:text-3xl mt-auto text-secondary">
                ${transaction.info.price}
              </h2>
            </div>
          )}
        ...
        <div className="flex gap-2 sm:flex-row flex-col">
          <div className="flex gap-2">
            {" "}
            <div className="border-2 border-outline rounded-md flex flex-col items-center relative w-[120px] sm:h-[200px] sm:w-[200px] overflow-hidden p-2 bg-gradient-to-tr from-outline via-white to-white">
              <h1
                className={`absolute top-1 left-1 sm:text-xl text-sm ${
                  transaction.seller.id == user.id
                    ? "text-green-400"
          ...
          {transaction.seller.id == user.id ? (
            <div className="text-black p-2 flex flex-col">
              <h1 className="sm:text-xl text-sm">Incoming Trade Accepted</h1>
              <h2 className="sm:text-lg text-xs">
                You traded your {transaction.info.original.api_data.name} for{" "}
                {transaction.buyer.username}'s{" "}
                {transaction.info.offer.api_data.name}
              </h2>
              <h2 className="sm:text-xl text-sm mt-auto text-green-400">
                You accepted {transaction.buyer.username}'s trade
              </h2>
            </div>
          ) : (
            <div className="text-black p-2 flex flex-col">
              <h1 className="sm:text-xl text-sm">Outgoing Trade Accepted</h1>
              <h2 className="sm:text-lg text-xs">
